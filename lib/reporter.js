...
var HtmlDetailedReporter = function (baseReporterDecorator, config, logger, helper, formatError) {
    ...
    function searchReplace(template, browser) {
        ...
        if (useHosted) {
            ...
            template = template.replace(/\{\{bootstrap_Path\}\}/g, 'https://stackpath.bootstrapcdn.com/bootstrap/4.3.1');
            template = template.replace(/\{\{fontawesome_Path\}\}/g, 'https://use.fontawesome.com/releases/v5.8.1');
        } else {
            ...
            template = template.replace(/\{\{fontawesome_Path\}\}/g, basePaths.fontawesome);
        }
    ...
    function getRelativePaths() {
        ...
        if (!bootstrap) log.error('Bootstrap is not found!');
            ...
            if (bootstrap.indexOf('/js') > -1) {
                bootstrap = bootstrap.substring(0, bootstrap.indexOf("/js"));  // <- This gives the path of the '/js' subfolder of bootstrap, so we need to go one level up (Mac/Linux)
            } else {
                bootstrap = bootstrap.substring(0, bootstrap.indexOf("\\js")).replace(/\\/g, '/');  // <- This gives the path of the '\js' subfolder of bootstrap, so we need to go one level up (Windows)
            }
            paths.bootstrap = bootstrap;
        }
        ...
        // Get fontawesome's path
        var fontawesome = require.resolve('@fortawesome/fontawesome-free');
        if (!fontawesome) log.error('FontAwesome is not found!');
        else {
            fontawesome = require('path').dirname(fontawesome);
            if (fontawesome.indexOf('/js') > -1) {
                fontawesome = fontawesome.substring(0, fontawesome.indexOf("/js"));
            } else {
                fontawesome = fontawesome.substring(0, fontawesome.indexOf("\\js")).replace(/\\/g, '/');
            }
            fontawesome = (splitResults ? path.relative(outputDir + '/tempFolder/', fontawesome) : path.relative(outputDir, fontawesome));
            paths.fontawesome = fontawesome;
        }
    ...
    function formatResults(browser) {
        ...
        header.ele('th', { 'style' : 'width:25px;' }, ' ');
        header.ele('th', { 'style' : 'width:100px;' }, 'Status');
        header.ele('th', 'Test');
        ...
        for (var i = 0; i < browserResults.length; i++) {
            ...
            if (result.skipped) {
                skipCnt[browser]++;
                row.att('class', 'alert-active' + (showSkipped ? '' : ' d-none'));
                ...
                passCnt[browser]++;
                row.att('class', 'alert-success' + (showSuccess ? '' : ' d-none'));
                statusCell.att('class', 'status');
                ...
                failCnt[browser]++;
                row.att('class', 'alert-danger' + (showFailed ? '' : ' d-none'));
                ...
                errorRow.att('class', 'alert-danger');
